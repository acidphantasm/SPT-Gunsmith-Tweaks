...
import PARTS from "../Gunsmith Tweaks/GunsmithParts.json";
    ...
    public postDBLoad(container: DependencyContainer): void {
        ...
        if (CONFIG.enabled) { // Enable or disable the mod
			...
			for (const GUNS in GUNSMITHGUNS) { 
				const questID = IDS[GUNS];
				const setupGunsmith = quests[questID].rewards.Started;
				const newGPReward = {
					"findInRaid": true,
					"id": GUNSMITHGUNS[GUNS].id, // I need a way to automate this
					"index": 0,
					"items": GUNSMITHGUNS[GUNS].items, // I need a way to automate this
					"target": GUNSMITHGUNS[GUNS].id, // I need a way to automate this
					"type": QuestRewardType.ITEM,
					"unknown": false,
					"value": 1
				}
				setupGunsmith.push(newGPReward); // I need a way to increase setupGunsmith & newGReward index by 1 for each item in the array skipping 2
			}
            ...
            if (CONFIG.LoreAccurate) { // If Lore Accurate is enabled, apply the changes
					...
					// This is a mess, I need to find a way to automate this
					// Move all parts to a separate file and import them here to make it easier to manage and update in the future ( GunsmithParts.json )
                    ...
                    const setupGunsmithP21M1911 = quests[IDS.GunsmithPart21M1].rewards.Started;
